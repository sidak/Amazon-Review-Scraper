...
exports.scrapeWithReviewerLink = function(name, id, ref, page, cb) {
    // The url, scripts, done keywords are optional 
    // but if you use them enclose them in curly braces 
    jsdom.env({
        url: "http://www.amazon.com/"+name+"/product-reviews/" + id +
        ...
        scripts: ["http://code.jquery.com/jquery.js"],
			...
			var $ = window.jQuery;
			var upvotesList=[], totalVotesList=[], ratingList=[], reviewDatesList=[], reviewerLinksList=[];
			...
			$("div#cm_cr-review_list")
				...
				.each(function() {
					...
					totalVotesList.push(totalVotes);
					ratingList.push(rating);
					reviewerLinksList.push(reviewerPageLink);
					reviewDatesList.push(reviewDate);
			...
			window.close();
			var result=[];
			result.push(upvotesList, totalVotesList, ratingList, reviewerLinksList, reviewDatesList);
			cb(null,result);
		}
    });
}
