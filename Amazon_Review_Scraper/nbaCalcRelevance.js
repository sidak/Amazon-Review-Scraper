...
var nbaData= require('./Input_files/nbaAmazonDataFormat_Feb2016_withReviewerLink');
var fs = require('fs');
...
var collectPageRatingStep= function (ct,game, idx, cb) {
	var normCt= (nbaData[game][idx][KEY_PAGES]-ct)+1;
	...
	//console.log(nbaData[game]);
	scraper.scrapeWithReviewerLink(nbaData[game][idx][KEY_NAME],nbaData[game][idx][KEY_ID], ref , pageNumber, function(err, result){
		if(err){
			...
			//console.log(nbaData[game][idx]);
			
			//Concatenate the initial votes list with the one scraped now
			//nbaData[game][idx][KEY_UVLIST]=nbaData[game][idx][KEY_UVLIST].concat(result[0]);
			//nbaData[game][idx][KEY_TVLIST]=nbaData[game][idx][KEY_TVLIST].concat(result[1]);
			//nbaData[game][idx][KEY_RLIST]=nbaData[game][idx][KEY_RLIST].concat(result[2]);
			nbaData[game][idx][KEY_RlLIST]=nbaData[game][idx][KEY_RlLIST].concat(result[3]);
			
			nbaData[game][idx][KEY_DLIST]=nbaData[game][idx][KEY_DLIST].concat(result[4]);
...
function scrapeSingleReviewerStep(ct, game, idx, cb){
	var numReviews = nbaData[game][idx][KEY_RlLIST].length;
	var reviewIdx = numReviews - ct;
	var reviewerPageLink = nbaData[game][idx][KEY_RlLIST][reviewIdx];
	...
	scraper.scrapeRanking(reviewerPageLink, function(err, result){
		if(err){
			...
			nbaData[game][idx][KEY_RRLIST].push(result);
			cb(null, result);
		}
	});
...
function scrapeReviewerRankingForPlatform(game, idx, cb){
	...
	asyncTraversal1(numReviews, game, idx, scrapeSingleReviewerStep, function(err, result){
		if(err!=null){
			...
			cb(null, nbaData[game][idx][KEY_RRLIST]);
		}
	});
...
function scrapeReviewerRankingForPlatform1(game, idx, cb){
	...
	for(var i=0; i<numReviews; i++){
		var reviewerPageLink = nbaData[game][idx][KEY_RlLIST][i];
		scraper.scrapeRanking(reviewerPageLink, function(err, result){
			if(err){
				console.log("error in single review scraper step");
				cb(err);
...
function collectNbaYearRatings(game, cb){
...
function collectGameReviewerRankings(cb){
	...
	collectNbaYearRatings(games[0], function(err, result){
		if(err){
			...
			collectNbaYearRatings(games[1], function(err, result){
				if(err){
...
function collectPlatformRatings(game, cb){
	for(var j=0; j<NUM_PLATFORMS; j++){
		(function(){
			...
			asyncTraversal(nbaData[game][idx][KEY_PAGES],game, idx, collectPageRatingStep, function(err, result){
				if(err)cb(err);
				else if (result!=null){
					cb(null,result);
				}
...
function writeDataForGamePlatform(game, idx, cb){
	...
	var gamePlatform= nbaData[game][idx];
	...
	if(err){
		...
		collectGameReviewerRankings(function(err, result){
			if(err)console.log(err);
				...
				//console.log(result);
				console.log('\n\n');
				//console.log(nbaData[KEY_NBA14]);
				//console.log(nbaData[KEY_NBA15]);
